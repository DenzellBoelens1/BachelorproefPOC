...
namespace Webshop.Backend.Middleware
{
    public class ProductWebSocketMiddleware
    {
        ...
        public async Task InvokeAsync(HttpContext context, ProductService productService)
        {
            ...
            if (!context.Request.Path.StartsWithSegments("/ws/product")
            ...
            else if (message.StartsWith("getProductById"))
            {
                ...
                {
                    response = JsonSerializer.Serialize(new { error = "Invalid product ID." });
                }
            }
            else if (message.StartsWith("updateStock"))
            {
                ...
                {
                    response = JsonSerializer.Serialize(new { error = "Invalid updateStock command format." });
                }
            }
            else if (message.StartsWith("getProductDetailsById"))
            {
                ...
                {
                    response = JsonSerializer.Serialize(new { error = "Invalid product details command format." });
                }
            }
            else if (message.StartsWith("calculatePrice:"))
            {
                try
                {
                    ...
                    var selectedOptionIds = new System.Collections.Generic.List<int>();
                    foreach (var el in root.GetProperty("selectedOptionIds").EnumerateArray())
                        selectedOptionIds.Add(el.GetInt32());

                    var optionValues = new System.Collections.Generic.Dictionary<int, string>();
                    ...
                    string? customText = root.TryGetProperty("customText", out var ctProp) && ctProp.ValueKind == JsonValueKind.String
                        ? ctProp.GetString()
                        : null;

                    var (unit, total) = await productService.CalculatePriceAsync(
                        productId,
                        quantity,
                        selectedOptionIds,
                        optionValues,
                    ...
                    response = JsonSerializer.Serialize(new PriceDTO { UnitPrice = unit, TotalPrice = total });
                }
                catch
                {
                    response = JsonSerializer.Serialize(new { error = "Invalid calculatePrice payload." });
                }
            }
            else
            {
